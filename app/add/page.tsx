...
export default function AddProject() {
  ...
  // Helper: Add a new blank team member
  const addTeamMember = () => {
    setTeamMembers([
      ...teamMembers, 
      { id: crypto.randomUUID(), fullName: '', role: '', xUsername: '', telegramUsername: '', linkedinUsername: '' }
    ]);
  };

  // Helper: Update a specific field for a team member
  const updateTeamMember = (id: string, field: keyof TeamMember, value: string) => {
    setTeamMembers(teamMembers.map(m => m.id === id ? { ...m, [field]: value } : m));
  };

  // Helper: Remove a team member
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      router.push('/'); // Go back to the dashboard
    }
    setLoading(false);
  };
  ...
  return (
    <div className="fixed inset-0 bg-[#0E1A22]/40 dark:bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      {/* Modal Container: Updated bg-white to dark:bg-[#0B1116] */}
      <div className="bg-white dark:bg-[#0B1116] w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="px-8 py-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-start bg-white dark:bg-[#0B1116]">
          ...
          <Link href="/">
            <button className="text-gray-300 hover:text-gray-500 dark:hover:text-gray-400 transition">
              <X size={20} />
            </button>
          </Link>
        </div>
          ...
          <form id="add-project-form" onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
              ...
              <input 
                ...
                className="w-full bg-[#F8FAFC] dark:bg-[#151F26] border border-gray-100 dark:border-gray-700 rounded-lg px-4 py-3 text-sm text-[#0E1A22] dark:text-white placeholder-gray-300 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#00E0FF]/20 focus:border-[#00E0FF] transition"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                ...
                <div className="relative">
                    ...
                    className="w-full bg-[#F8FAFC] dark:bg-[#151F26] border border-gray-100 dark:border-gray-700 rounded-lg pl-10 pr-4 py-3 text-sm text-[#0E1A22] dark:text-white placeholder-gray-300 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#00E0FF]/20 focus:border-[#00E0FF] transition"
                    placeholder="https://..."
                    />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                    ...
                    <input 
                    value={projectX}
                    onChange={(e) => setProjectX(e.target.value)}
                    className="w-full bg-[#F8FAFC] dark:bg-[#151F26] border border-gray-100 dark:border-gray-700 rounded-lg pl-10 pr-4 py-3 text-sm text-[#0E1A22] dark:text-white placeholder-gray-300 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#00E0FF]/20 focus:border-[#00E0FF] transition"
                    placeholder="handle"
                    />
                </div>
            ...
            <div>
              ...
              <div className="space-y-4 mb-4">
                {teamMembers.map((member) => (
                  <div key={member.id} className="bg-white dark:bg-[#151F26] border border-gray-100 dark:border-gray-700 rounded-xl p-5 shadow-sm relative group hover:border-gray-200 dark:hover:border-gray-600 transition">
                    <button type="button" onClick={() => removeTeamMember(member.id)} className="absolute top-4 right-4 text-gray-200 hover:text-red-400 transition">
                      <X size={16} />
                    </button>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                         ...
                         <input 
                            ...
                            className="w-full bg-[#F8FAFC] dark:bg-[#0B1116] border border-gray-100 dark:border-gray-700 rounded px-3 py-2 text-sm text-[#0E1A22] dark:text-white focus:outline-none focus:border-[#00E0FF]"
                            placeholder="Name"
                         />
                      </div>
                      <div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div>
                             <label className="block text-[9px] font-bold text-gray-300 dark:text-gray-500 uppercase mb-1">X (Twitter)</label>
                             <input value={member.xUsername} onChange={(e) => updateTeamMember(member.id, 'xUsername', e.target.value)} className="w-full bg-[#F8FAFC] dark:bg-[#0B1116] border border-gray-100 dark:border-gray-700 rounded px-3 py-2 text-xs text-[#0E1A22] dark:text-white placeholder-gray-300 dark:placeholder-gray-600 focus:outline-none focus:border-[#00E0FF]" placeholder="@handle" />
                        </div>
                        <div>
                             <label className="block text-[9px] font-bold text-gray-300 dark:text-gray-500 uppercase mb-1">Telegram</label>
                             <input value={member.telegramUsername} onChange={(e) => updateTeamMember(member.id, 'telegramUsername', e.target.value)} className="w-full bg-[#F8FAFC] dark:bg-[#0B1116] border border-gray-100 dark:border-gray-700 rounded px-3 py-2 text-xs text-[#0E1A22] dark:text-white placeholder-gray-300 dark:placeholder-gray-600 focus:outline-none focus:border-[#00E0FF]" placeholder="@user" />
                        </div>
                        <div>
                             <label className="block text-[9px] font-bold text-gray-300 dark:text-gray-500 uppercase mb-1">LinkedIn</label>
                             <input value={member.linkedinUsername} onChange={(e) => updateTeamMember(member.id, 'linkedinUsername', e.target.value)} className="w-full bg-[#F8FAFC] dark:bg-[#0B1116] border border-gray-100 dark:border-gray-700 rounded px-3 py-2 text-xs text-[#0E1A22] dark:text-white placeholder-gray-300 dark:placeholder-gray-600 focus:outline-none focus:border-[#00E0FF]" placeholder="in/username" />
                        </div>
                    </div>
                  </div>
                ))}
              ...
              <button 
                ...
                className="w-full py-3 border border-dashed border-gray-200 dark:border-gray-700 rounded-xl text-gray-400 dark:text-gray-500 text-xs font-bold uppercase tracking-wide hover:border-[#00E0FF] hover:text-[#00E0FF] transition flex items-center justify-center gap-2 bg-[#F8FAFC]/50 dark:bg-[#151F26]/50"
              >
            ...
            <div className="bg-[#F8FAFC] dark:bg-[#151F26] rounded-xl p-5 flex justify-between items-center border border-gray-100 dark:border-gray-700">
              ...
              <div className="flex bg-white dark:bg-[#0B1116] rounded-lg p-1 border border-gray-100 dark:border-gray-700 shadow-sm">
                <button type="button" onClick={() => setDiscordTicket(true)} className={`px-5 py-1.5 rounded text-xs font-bold transition ${discordTicket ? 'bg-[#00E0FF] text-[#0E1A22]' : 'text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300'}`}>Yes</button>
                <button type="button" onClick={() => setDiscordTicket(false)} className={`px-5 py-1.5 rounded text-xs font-bold transition ${!discordTicket ? 'bg-gray-100 dark:bg-gray-800 text-[#0E1A22] dark:text-white' : 'text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300'}`}>No</button>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea 
                ...
                className="w-full bg-[#F8FAFC] dark:bg-[#151F26] border border-gray-100 dark:border-gray-700 rounded-lg px-4 py-3 text-sm text-[#0E1A22] dark:text-white placeholder-gray-300 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#00E0FF]/20 focus:border-[#00E0FF] transition h-32 resize-none"
                placeholder="Internal notes, context, or deal observations."
              />
            </div>
        ...
        <div className="px-8 py-5 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-[#0B1116] flex justify-end gap-3 sticky bottom-0 z-10">
          ...
          <button 
            ...
            disabled={loading}
            className="px-8 py-2.5 bg-[#00E0FF] text-[#0E1A22] text-sm font-bold rounded-full hover:shadow-lg hover:shadow-[#00E0FF]/20 hover:brightness-105 transition disabled:opacity-50"
          >
